<script>
   ...
   import { SHIELD_TRAITS } from '~/document/types/item/types/shield/ShieldTraits.js';

   /**
    * @typedef {object} ShieldTraitSelectProps
   ...
   const options = $derived.by(() => {
      const list = SHIELD_TRAITS.map((trait) => trait.name);
      if (allowNone) {
         list.push('none');
      }
      return list;
