 ...
 * followed by the first owning player. This is used to ensure certain functions only fire once, such as when an
 * actor's turn starts.
...
export default function isCurrentUserBestOwner(document) {
   ...
   // Otherwise check if there is an active owner. If so, return if the current user is the first active owner.
