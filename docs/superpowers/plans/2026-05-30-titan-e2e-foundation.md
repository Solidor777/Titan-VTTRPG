...
```bash
git add src/quench/batches/player-sheet-render.batch.js src/quench/RegisterQuenchTests.js tests/e2e/quench-runner.spec.js
git commit -m "test(e2e): Quench->Playwright signal bridge; split batch registrar"
```

> `index.js` / `index.js.map` / `style.css` are gitignored build output — never `git add` them; they are regenerated by `npm run build`.
...
```bash
git add src/helpers/svelte-components/button/Button.svelte src/helpers/svelte-components/input/TextInput.svelte tests/components/ButtonProbe.svelte tests/unit/test-id-prop.test.js
